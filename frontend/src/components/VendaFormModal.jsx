...
export default function VendaFormModal({ open, onClose, onSaved, lead }) {
  ...
  const [form, setForm] = useState({ produto_id: '', valor: '', forma_pagamento: '', data_venda: hoje(), observacoes: '' });
  ...
  const semProduto = produtos.length === 0;
    ...
    >
      {semProduto ? (
        <p className="text-sm text-gray-600 dark:text-gray-300">
          Você ainda não tem produtos ativos. Cadastre um produto em <strong>Produtos</strong> antes de registrar a venda —
          é ele que define a regra de comissão.
        </p>
      ) : (
